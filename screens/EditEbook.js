...
const EditbookScreen = ({ navigation, image, userId }) => {
  ...
  const saveEdit = async (profilePictureUrl) => {
    if (editingUser) {
      try {
        await BookCollection.doc(editingUser).update({
          name: newName,
          Author: newAuthor,
  ...
  const pickImage = async () => {
    ...
    if (!result.canceled) {
      try {
        ...
        const ref = firebase.storage().ref().child(`bookpicture/${imageName}`);
  ...
  const handleImageUrlChange = async () => {
    if (imageUrl) {
      ...
      if (editingUser) {
        try {
          await firebase.firestore().collection('book').doc(editingUser).update({ image: imageUrl });
          setImageUrl('');
        } catch (error) {
